...
# main Class
class main:
    def __init__(self,master,test=True):
        # Window
        self.master = master
    ...
    def login(self):
        ...
        user = self.getUser()
        if user == None:  # AviChange
            ms.showerror("אוי", "שם המשתמש או הסיסמא לא תקינים")
        else:
            print ('### User record: ')
            print (user)
            userName = user[0]
            userRole = user[3].upper()
            userClass = user[5]

            self.logf.pack_forget() # Avi ???
            ...
            if userRole == 'TEACHER':
                ...
                #print(f"לא מכיר את התפקיד {userName} - {userRole}")
                #return
        return user
